...
import { SaPlaygroundItem, MStructureType } from "../../type";

const MIN_WIDTH = 520;

export const useFunctionHooks = (
  emit: any,
  baseConfig: Ref<SaPlaygroundItem | undefined>,
  dataStructures: Ref<MStructureType[]>,
  tableConfig: Ref<SaStructureType.TableV2[]>,
  tableConfigExOperation: Ref<SaStructureType.TableV2[]>,
  tableExOptions: Ref<Record<string, string>>,
  tableData: Ref<any[][]>,
  tableWidth: Ref<number>,
  editHeadersData: Ref<SaPlaygroundPageButtonType[]>,
  editOperationData: Ref<SaPlaygroundPageButtonType[]>,
  ...
  // 更新列
  function updateCol(col: SaStructureType.TableV2 & { cellType: any }, options: Record<string, string>) {
  ...
  // 更新所有列
  function updateColAll(cols: SaStructureType.TableV2[], options?: Record<string, string>) {
  ...
  function exportTableConfig() {
    const _tableConfig: SaStructureType.TableV2[] = [];
  ...
  function handleRefresh() {
    ...
    if (oldConfig && findConfig?.config) {
      const outData: SaStructureType.TableV2[] = [];
