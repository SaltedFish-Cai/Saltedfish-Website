...
export type MRadioType = {
   ...
   * @description 该值的类型为 `number` | `string` | `boolean`，可以是任意类型
   * @example
   * ```tsx
   * <MRadio contrastData={1} />
   * ```
   * */
   ...
   * @example
   * ```tsx
   * <MRadio onChange={({value, oldValue}) => { console.log(value, oldValue) }} />
   * ```
   * */
